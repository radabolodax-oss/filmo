...
const TMDB_GENRES: Record<number, string> = {
  ...
  80: 'Crime', 18: 'Drame', 10751: 'Famille', 14: 'Fantastique',
  36: 'Histoire', 27: 'Horreur', 9648: 'Mystère', 10749: 'Romance',
  878: 'Science-Fiction', 53: 'Thriller', 10752: 'Guerre', 37: 'Western',
  10759: 'Action & Aventure', 10765: 'Sci-Fi & Fantastique', 99: 'Documentaire',
};
...
const Home: React.FC = () => {
  ...
  // Fetch featured spotlight — random pick from popular recent movies + anime
  useEffect(() => {
    const fetchFeaturedSeries = async () => {
      try {
        ...
        const [trendingMovies, animeMovies, animeTv] = await Promise.allSettled([
          // Films tendance de la semaine
          axios.get('https://api.themoviedb.org/3/trending/movie/week', {
            params: { api_key: TMDB_API_KEY, language: getTmdbLanguage() },
          }),
          // Films d'animation récents
        ...
        if (trendingMovies.status === 'fulfilled') {
          trendingMovies.value.data.results.forEach((m: any) => {
            if (m.vote_average >= 7 && m.vote_count >= 100 && m.backdrop_path) {
              pool.push({ ...m, media_type: 'movie' });
            }
          });
        }
        if (animeMovies.status === 'fulfilled') {
          animeMovies.value.data.results.forEach((m: any) => {
            if (m.vote_average >= 6.5 && m.backdrop_path) {
              pool.push({ ...m, media_type: 'movie' });
            }
          });
        }
        if (animeTv.status === 'fulfilled') {
          animeTv.value.data.results.forEach((m: any) => {
            if (m.vote_average >= 6.5 && m.backdrop_path) {
              pool.push({ ...m, media_type: 'tv' });
            }
          });
        }
        ...
        // Déduplique et choisit aléatoirement
        const seen = new Set<number>();
        const unique = pool.filter(m => { if (seen.has(m.id)) return false; seen.add(m.id); return true; });
        const pick = unique[Math.floor(Math.random() * unique.length)];
        setFeaturedSeries(pick);
      } catch (error) {
        console.error('Error fetching featured spotlight:', error);
      }
    };
    fetchFeaturedSeries();
  }, []);
  ...
  const trendingTodayTitle = useMemo(
    () => <CarouselTitle icon="🔥" iconClass="text-green-400" label={t('home.trendingToday')} />,
    [t]
  );
  ...
  const becauseYouWatchedTitles = useMemo(
    () => (personalizedReco?.becauseYouWatched || []).map((group) =>
      <CarouselTitle icon="✨" iconClass="text-white" label={t('home.becauseYouWatched', { title: group.title })} />
    ),
  ...
  const platformsItems = useMemo(() => [
    ...
    { id: 384, src: 'https://upload.wikimedia.org/wikipedia/commons/thumb/b/b3/HBO_Max_%282025%29.svg/250px-HBO_Max_%282025%29.svg.png', video: 'https://media.tenor.com/7xmvr-fKGLMAAAAd/hbo-max-warner-bros-pictures.gif', alt: 'HBO MAX', route: '/provider/384', label: 'HBO MAX' },
  ], [t]);
    ...
    <div className="w-full min-h-screen bg-black text-white">
      ...
      >
        ...
        {loading ? (
          ...
          <>
            {heroItems.length > 0 && (
              <div className="relative w-full mb-2 mt-[304px]">
            ...
            {continueWatching.length > 0 && (
              <div className="content-row-container px-4 md:px-8 mb-0 mt-2">
            ...
            {personalizedReco?.becauseYouWatched?.map((group, idx) => (
              <div key={`byw-${group.sourceId}`} className="content-row-container px-4 md:px-8 mb-0 mt-2">
            ...
            {personalizedReco?.topGenres?.map((group, idx) => (
              <div key={`genre-${group.genreId}`} className="content-row-container px-4 md:px-8 mb-0 mt-2">
            ...
            {/* Plateformes de streaming - avant Tendances du jour */}
            <div className="content-row-container px-4 md:px-8 mb-0 mt-2">
            ...
            {/* Section "Tendances du jour" - Section prioritaire (index 2) */}
            {topContent.length > 0 && (
              <div className="content-row-container px-4 md:px-8 mb-0 mt-2">
            ...
            >
              {!loading && (
                <div>
                  ...
                  {/* Featured Spotlight — sélection aléatoire parmi films/animés populaires récents */}
                  {featuredSeries && (
                    <motion.div
                      initial={{ opacity: 0, y: 20 }}
                    ...
                    >
                      <div
                        className="w-full min-h-[400px] h-[65svh] max-h-[700px] relative overflow-hidden"
                      ...
                      >
                        ...
                        {/* Fades 4 côtés */}
                        <div className="absolute inset-y-0 left-0 w-16 sm:w-24 md:w-32 z-20 pointer-events-none bg-gradient-to-r from-black via-black/50 to-transparent" />
                        <div className="absolute inset-y-0 right-0 w-16 sm:w-24 md:w-32 z-20 pointer-events-none bg-gradient-to-l from-black via-black/50 to-transparent" />
                        <div className="absolute top-0 left-0 right-0 h-24 z-20 pointer-events-none bg-gradient-to-b from-black via-black/40 to-transparent" />
                        <div className="absolute bottom-0 left-0 right-0 h-40 z-20 pointer-events-none bg-gradient-to-t from-black via-black/50 to-transparent" />
                        <div className="flex items-start justify-center flex-col h-full z-20 relative gap-5 px-6 md:px-12 py-10">
                          ...
                          <div className="flex flex-row gap-2 items-center flex-wrap">
                            ...
                            {(featuredSeries.release_date || featuredSeries.first_air_date) && (
                              <span className="inline-flex items-center px-3 py-1 rounded-full bg-white/5 backdrop-blur-xl border border-white/20 text-white/90 text-xs font-medium">
                                {new Date(featuredSeries.release_date || featuredSeries.first_air_date).getFullYear()}
                              </span>
                            )}
                            <span className="inline-flex items-center px-3 py-1 rounded-full bg-white/5 backdrop-blur-xl border border-white/20 text-white/90 text-xs font-medium">
                              {featuredSeries.media_type === 'movie' ? t('search.movieLabel') : t('search.serieLabel')}
                            </span>
                            {(featuredSeries.genre_ids ?? []).slice(0, 3).map((gid: number) =>
                              TMDB_GENRES[gid] ? (
                                <span key={gid} className="inline-flex items-center px-3 py-1 rounded-full bg-white/5 backdrop-blur-xl border border-white/10 text-white/80 text-xs font-medium">
                                  {TMDB_GENRES[gid]}
                                </span>
                              ) : null
                            )}
                          ...
                          <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
                            <Link
                              to={`/${featuredSeries.media_type}/${encodeId(featuredSeries.id)}`}
                  ...
                  {categories.map((category, catIndex) => (
                    <div key={category.id} className="mb-3 px-4 md:px-8">
