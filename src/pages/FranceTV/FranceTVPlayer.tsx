    ...
    >
      ...
      {error && (
        <div className="absolute inset-0 z-30 flex flex-col items-center justify-center bg-black/90 px-4">
          <AlertCircle className="w-14 h-14 text-green-400 mb-4" />
          <p className="text-green-400 text-center mb-6 max-w-lg text-sm">{error}</p>
          <button onClick={() => navigate(-1)}
      ...
      >
        <div className="max-w-screen-2xl mx-auto">
          ...
          >
            {/* Progress (red) */}
            <div
              className="absolute inset-y-0 left-0 bg-gradient-to-r from-green-400 to-purple-500 rounded-full"
              style={{ width: duration ? `${(currentTime / duration) * 100}%` : '0%' }}
            />
            {/* Handle */}
            <div
              className="absolute top-1/2 -translate-y-1/2 w-4 h-4 bg-gradient-to-r from-green-400 to-purple-500 rounded-full -ml-2 opacity-0 group-hover:opacity-100 transition-opacity shadow-lg"
                    ...
                    {qualities.map(q => (
                      <button
                        ...
                        className={`w-full text-left px-3 py-2.5 rounded-lg text-sm transition-colors cursor-pointer ${
                          selectedQuality === q.value ? 'bg-gradient-to-r from-green-400 to-purple-500 text-green-400' : 'text-gray-300 hover:bg-white/5'
                        }`}
                      >
                        {q.label}
                        {selectedQuality === q.value && <span className="float-right text-green-400">✓</span>}
                      </button>
                    ))}
                  </motion.div>
                )}
                  ...
                  <motion.div key="audio" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.25 }}>
                    {audioTracks.map(a => (
                      <button
                        ...
                        className={`w-full text-left px-3 py-2.5 rounded-lg text-sm transition-colors cursor-pointer ${
                          selectedAudio === a.value ? 'bg-gradient-to-r from-green-400 to-purple-500 text-green-400' : 'text-gray-300 hover:bg-white/5'
                        }`}
                      >
                        {a.label}
                        {selectedAudio === a.value && <span className="float-right text-green-400">✓</span>}
                      </button>
                    ))}
                  </motion.div>
                )}
                  ...
                  <motion.div key="subtitles" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.25 }}>
                    {subtitles.map(s => (
                      <button
                        ...
                        className={`w-full text-left px-3 py-2.5 rounded-lg text-sm transition-colors cursor-pointer ${
                          selectedSub === s.value ? 'bg-gradient-to-r from-green-400 to-purple-500 text-green-400' : 'text-gray-300 hover:bg-white/5'
                        }`}
                      >
                        {s.label}
                        {selectedSub === s.value && <span className="float-right text-green-400">✓</span>}
                      </button>
                    ))}
                  </motion.div>
                )}
                  ...
                  <motion.div key="speed" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.25 }}>
                    {[0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2].map(speed => (
                      <button
                        ...
                        className={`w-full text-left px-3 py-2.5 rounded-lg text-sm transition-colors cursor-pointer ${
                          playbackSpeed === speed ? 'bg-gradient-to-r from-green-400 to-purple-500 text-green-400' : 'text-gray-300 hover:bg-white/5'
                        }`}
                      >
                        {speed === 1 ? t('watch.normalLabel') : `${speed}x`}
                        {playbackSpeed === speed && <span className="float-right text-green-400">✓</span>}
                      </button>
                    ))}
                  </motion.div>
                )}
