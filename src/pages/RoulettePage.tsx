...
const fireConfetti = async () => {
  ...
  const colors = ['#ef4444', '#f59e0b', '#10b981', '#3b82f6', '#ffffff', '#ec4899'];
...
const SpinningReel: React.FC<{
  ...
  return (
    <div ref={containerRef} className="relative w-full overflow-hidden rounded-2xl border border-white/10 bg-black/40 backdrop-blur-sm" style={{ height: CONTAINER_HEIGHT }}>
      {/* Center marker */}
      <div className="absolute left-1/2 top-2 bottom-2 z-20 pointer-events-none border-2 border-white/20 rounded-xl shadow-[0_0_30px_rgba(239,68,68,0.3)]"
...
const VerticalReel: React.FC<{
  ...
  return (
    ...
    >
      <div
        className="absolute left-1 right-1 z-20 pointer-events-none border-2 border-white/20 rounded-lg shadow-[0_0_20px_rgba(239,68,68,0.4)]"
    ...
    >
      ...
      {reelsVisible && (
        <div className="flex items-center justify-center gap-3 mb-3">
          <Dices className="w-5 h-5 text-green-400 animate-spin" />
          <span className="text-sm font-semibold text-white/70">
            {t('roulette.roundProgress', { round })}
          </span>
          <div className="flex gap-1.5">
            <div className={`w-2 h-2 rounded-full transition-colors ${isRound1Done ? 'bg-green-500' : 'bg-gradient-to-r from-green-400 to-purple-500 animate-pulse'}`} />
            <div className={`w-2 h-2 rounded-full transition-colors ${round === 2 ? 'bg-gradient-to-r from-green-400 to-purple-500 animate-pulse' : 'bg-white/20'}`} />
          </div>
        </div>
      )}
        ...
        <div className="max-w-screen-xl mx-auto">
          ...
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
            <div className="flex items-center justify-center gap-3 mb-2">
              <Dices className="w-8 h-8 text-green-400" />
                      ...
                      <div className="flex flex-wrap gap-1.5 mt-2">
                        {selectedGenres.map(id => (
                          <motion.button key={id} initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }}
                            ...
                            className="flex items-center gap-1 px-2 py-1 rounded-lg bg-red-500/20 border border-white/20 text-red-300 text-xs hover:bg-red-500/30 transition-colors">
                      ...
                      <div className="flex flex-wrap gap-1.5 mt-2">
                        {selectedProviders.map(id => (
                          <motion.button key={id} initial={{ scale: 0 }} animate={{ scale: 1 }}
                            ...
                            className="flex items-center gap-1 px-2 py-1 rounded-lg bg-white/10 border border-white/20 text-white text-xs hover:bg-white/10 transition-colors">
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex justify-center gap-3 mb-6">
                ...
                {spinCount > 1 && (
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    onClick={skipAll}
                    className="px-4 py-2 rounded-xl bg-white/5 border border-white/10 hover:bg-red-500/20 hover:border-white/20 text-white/50 hover:text-white text-sm transition-all flex items-center gap-2"
                  >
                    <ChevronDown className="w-4 h-4" />
                    ...
                    <div className="flex items-center gap-3 mt-auto">
                      <Link to={`/${winner.media_type}/${encodeId(winner.id)}`}>
                        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                          className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-green-400 to-purple-500 hover:bg-gradient-to-r hover:from-green-400 to-purple-500 text-white font-medium transition-colors">
