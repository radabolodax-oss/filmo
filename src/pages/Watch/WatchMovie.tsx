...
const WatchMovie: React.FC = () => {
  ...
  // ——— Métadonnées d'affichage pour l'en-tête (année, note, durée, certification) ———
  const [movieMeta, setMovieMeta] = useState<{ release_date: string; vote_average: number; runtime: number; overview: string; genres: { id: number; name: string }[] } | null>(null);
  const [displayCert, setDisplayCert] = useState<string>('');
  // ——— Détection anime (films d'animation) — même heuristique que WatchTv/TVDetails,
  // mais via les endpoints /movie de TMDB (keywords y a une forme différente de /tv). ———
  ...
  // FrAnime) — on retombe alors sur le lecteur film classique. Réinitialisé par film.
  const [animeSourceFallback, setAnimeSourceFallback] = useState(false);
  useEffect(() => { setAnimeSourceFallback(false); }, [id]);
  useEffect(() => {
    if (!id) return;
    let cancelled = false;
    axios.get(`https://api.themoviedb.org/3/movie/${id}`, { params: { api_key: TMDB_API_KEY, language: getTmdbLanguage(), append_to_response: 'keywords' } })
      .then(r => {
        if (cancelled) return;
        setMovieMeta({ release_date: r.data.release_date, vote_average: r.data.vote_average, runtime: r.data.runtime, overview: r.data.overview, genres: r.data.genres || [] });
        // /movie/{id}/keywords renvoie { keywords: [...] } (pas { results: [...] } comme /tv/{id}/keywords).
        const keywords: TmdbKeywordsResponse = { results: r.data?.keywords?.keywords ?? [] };
        setAnimeMode(!shouldHideAnimeModeForId(id) && isLikelyAnime(r.data, keywords));
      })
      .catch(() => {});
    axios.get(`https://api.themoviedb.org/3/movie/${id}/release_dates`, { params: { api_key: TMDB_API_KEY } })
      .then(r => {
        if (cancelled) return;
        const results = r.data.results;
        let cert = '';
        const frRelease = results.find((x: any) => x.iso_3166_1 === 'FR');
        ...
        if (!cert) {
          const usRelease = results.find((x: any) => x.iso_3166_1 === 'US');
  ...
  // ——— Films similaires, affichés sous l'encadré du player ———
  const [recommendations, setRecommendations] = useState<any[]>([]);
  useEffect(() => {
    if (!id) return;
    let cancelled = false;
    axios.get(`https://api.themoviedb.org/3/movie/${id}/recommendations`, { params: { api_key: TMDB_API_KEY, language: getTmdbLanguage() } })
  ...
  const fetchVideoSources = async () => {
    ...
    try {
      ...
      // Même pattern que WatchTv.tsx : ping via le proxy backend (CORS) avec timeout
      // court pour savoir si le film existe côté Wavewatch avant de l'auto-sélectionner.
      const wavewatchPromise: Promise<string | null> = (async () => {
        const wwUrl = `https://wwembed.wavewatch.top/api/v1/streaming/ww-movie-${id}`;
      ...
      const applyEmbedConfig = async (sourceId: TopLevelSourceId): Promise<boolean> => {
        switch (sourceId) {
          ...
          case 'wavewatch': {
            ...
            // Convention existante (cf. handleSourceChange dans HLSPlayer) : les lecteurs
            // VO/VOSTFR top-level, Wavewatch inclus, partagent le type générique 'vostfr'
            // côté embed — c'est l'URL qui les distingue, pas selectedSource/embedType.
          ...
          // Les ids restants (vox, rivestream_hls) ne sont pas
          // auto-sélectionnables dans le flux Movie (vox = WatchTv uniquement,
          // rivestream_hls déprécié).
          default:
            return false;
        }
      };
  ...
  if (animeMode && !animeSourceFallback) {
    return (
      <WatchAnime
        ...
        headerYear={movieMeta?.release_date && !isNaN(new Date(movieMeta.release_date).getTime()) ? new Date(movieMeta.release_date).getFullYear() : undefined}
        headerVoteAverage={movieMeta?.vote_average}
  ...
  return (
    <div className="w-full max-w-4xl mx-auto my-6">
      ...
      <div className="mb-4 flex flex-col items-center text-center gap-3">
        <h1 className="text-2xl md:text-3xl font-bold text-white">{movieTitle}</h1>
        <div className="flex flex-wrap items-center justify-center gap-3">
          {movieMeta?.release_date && !isNaN(new Date(movieMeta.release_date).getTime()) && (
            <span className="flex items-center gap-1.5 text-white/60 text-sm">
              <span>📅</span>
              <span>{new Date(movieMeta.release_date).getFullYear()}</span>
            </span>
          )}
          {displayCert && (
          ...
          {!!movieMeta?.vote_average && movieMeta.vote_average > 0 && (
            <span className="flex items-center gap-1 text-white/60 text-sm">
              <span className="text-yellow-500">⭐</span>
              <span className="font-bold text-white/80">{movieMeta.vote_average.toFixed(1)}</span>
              <span className="text-xs">/10</span>
            </span>
          )}
          {!!movieMeta?.runtime && movieMeta.runtime > 0 && (
            <span className="flex items-center gap-1.5 text-white/60 text-sm">
              <span>🕐</span>
              <span>{Math.floor(movieMeta.runtime / 60)}h{String(movieMeta.runtime % 60).padStart(2, '0')}</span>
            </span>
          )}
          {movieMeta?.release_date && !isNaN(new Date(movieMeta.release_date).getTime()) ? (
            new Date(movieMeta.release_date) > new Date() ? (
              <span
          ...
          <AnimatePresence>
            {showEmbedQuality && (
              <motion.div
                ...
                className="absolute inset-0 z-[10000] bg-black/50 flex justify-end pointer-events-auto"
              >
      ...
      {/* Encadré infos du film sélectionné — sous le player, au-dessus des recommandations */}
      <div className="mt-6 rounded-2xl border border-white/10 bg-white/[0.03] p-6 flex flex-col sm:flex-row sm:items-stretch gap-6">
        ...
        <div className="flex-1 min-w-0">
          {movieMeta?.overview && (
            <p className="text-white/60 text-sm md:text-base leading-relaxed">{movieMeta.overview}</p>
          )}
        </div>
      </div>
      ...
      {recommendations.length > 0 && (
        <div className="mt-8">
          <div className="mb-4 flex items-center justify-center gap-2">
            ...
            <h3 className="text-xl font-bold text-white">Films similaires</h3>
          </div>
