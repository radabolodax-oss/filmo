...
const WatchMovie: React.FC = () => {
  ...
  const [similarMovies, setSimilarMovies] = useState<{ id: number; title?: string; name?: string; poster_path: string; vote_average: number; media_type?: string }[]>([]);
  ...
  const fetchVideoSources = async () => {
    ...
    try {
      ...
      const coflixPromise = axios.get(`${MAIN_API}/api/tmdb/movie/${id}`, { timeout: 15000 })
      ...
      const wiflixPromise: Promise<WiflixMovieResponse | null> = axios.get(`${MAIN_API}/api/wiflix/movie/${id}`, { timeout: 30000 })
      ...
      const viperPromise: Promise<ViperMovieResponse | null> = axios.get(`${MAIN_API}/api/cpasmal/movie/${id}`, { timeout: 30000 })
      ...
      console.log('🇫🇷 [VF] FStream HLS extraites:', fstreamHlsSources.length);
      ...
      console.log('📦 [OMEGA] Omega disponible:', !!(omegaResult && getSupervideoFromOmega(omegaResult)));
      console.log('📦 [CUSTOM] Custom links:', customLinks.length);
      console.log('📦 [FREMBED] Frembed disponible:', isFrembedAvailable);
      ...
      // Special case for movie ID 1311031 - force custom source selection
        ...
        const vfAvailable = vfCandidates.filter(id => availability.find(a => a.id === id && a.hasData));
        ...
        if (vfAvailable.length > 0) {
          ...
          applyVfConfigRef.current = async () => {
            for (const vfId of vfAvailable) {
              if (await applyEmbedConfig(vfId)) break;
            }
  ...
  // Fetch similar movies for end-of-film overlay
  useEffect(() => {
    if (!id) return;
  ...
  // Auto-redirect vers le lecteur VOSTFR (Videasy) quand aucune source VF n'est trouvée
  useEffect(() => {
    if (onlyVostfrAvailable) {
      setOnlyVostfrAvailable(false);
    }
  }, [onlyVostfrAvailable]);
        ...
        <div className="w-full h-full flex items-center justify-center">
          <HLSPlayer
            ...
            onSimilarContentSelect={(contentId, mediaType) => {
              ...
              navigate(mediaType === 'tv' ? `/tv/${encoded}` : `/movie/${encoded}`);
            }}
          />
          {/* Sources Menu Overlay */}
          <AnimatePresence>
