...
import { shouldHideAnimeModeForId } from '../../utils/searchUtils';
...
const WatchTv: React.FC = () => {
  ...
  // Ensure IDs are valid before parsing. Saison/épisode ne sont plus dans l'URL
  // (route /tv/:id, comme le reste du site) — ce sont des state internes mis à jour
  // par handleNextEpisodeNav, plutôt qu'un rechargement complet de page.
  const [seasonNumber, setSeasonNumber] = useState(parseInt(seasonParam || '1', 10));
  const [episodeNumber, setEpisodeNumber] = useState(parseInt(episodeParam || '1', 10));

  // ——— Détection anime + métadonnées d'affichage (année, note, nb saisons, certification) ———
  // Même heuristique multi-signaux que TVDetails.tsx (origin_country/langue/studio JP/keywords).
  const [tvMeta, setTvMeta] = useState<{ first_air_date: string; vote_average: number; number_of_seasons: number; overview: string; genres: { id: number; name: string }[] } | null>(null);
  const [displayCert, setDisplayCert] = useState<string>('');
  const [tmdbKeywords, setTmdbKeywords] = useState<TmdbKeywordsResponse | null>(null);
  const [animeMode, setAnimeMode] = useState(false);
  // Passe à true si WatchAnime ne trouve ce titre sur aucun provider (AnimeSama/AniCloud/
  // FrAnime) — on retombe alors sur le lecteur TV classique au lieu de l'écran d'erreur
  // avec fallback Videasy/Peachify. Réinitialisé à chaque changement de show.
  const [animeSourceFallback, setAnimeSourceFallback] = useState(false);
  useEffect(() => { setAnimeSourceFallback(false); }, [id]);
  useEffect(() => {
    if (!id) return;
    let cancelled = false;
    axios.get(`https://api.themoviedb.org/3/tv/${id}`, {
      params: { api_key: TMDB_API_KEY, language: getTmdbLanguage(), append_to_response: 'keywords,content_ratings' },
    }).then(r => {
      if (cancelled) return;
      setTvMeta({
        first_air_date: r.data.first_air_date,
        vote_average: r.data.vote_average,
        number_of_seasons: r.data.number_of_seasons,
        overview: r.data.overview,
        genres: r.data.genres || [],
      });
      setTmdbKeywords(r.data?.keywords ?? null);
      setAnimeMode(!shouldHideAnimeModeForId(id) && isLikelyAnime(r.data, r.data?.keywords ?? null));
      const results = r.data?.content_ratings?.results || [];
      const frRating = results.find((x: any) => x.iso_3166_1 === 'FR');
      const usRating = results.find((x: any) => x.iso_3166_1 === 'US');
      const cert = frRating?.rating || usRating?.rating || '';
      if (cert) setDisplayCert(cert);
    }).catch(() => {});
    return () => { cancelled = true; };
  }, [id]);

  // Expose au Header si la page courante est un anime, pour que le bouton retour cible /anime
  useEffect(() => {
    (window as any).__currentIsAnime = animeMode;
    return () => { (window as any).__currentIsAnime = false; };
  }, [animeMode]);

  // ——— Séries similaires, affichées sous l'encadré du player ———
  const [recommendations, setRecommendations] = useState<any[]>([]);
  useEffect(() => {
    if (!id) return;
    let cancelled = false;
    axios.get(`https://api.themoviedb.org/3/tv/${id}/recommendations`, { params: { api_key: TMDB_API_KEY, language: getTmdbLanguage() } })
      .then(r => { if (!cancelled) setRecommendations(r.data?.results || []); })
      .catch(() => {});
    return () => { cancelled = true; };
  }, [id]);
  ...
  const handleNextEpisodeNav = (targetSeason: number, targetEpisode: number) => {
    ...
    setEpisodeNumber(targetEpisode);
  };
  ...
  useEffect(() => {
    ...
    const initialFetch = async () => {
      ...
      try {
        ...
        // ========== CHECK WAVEWATCH SOURCE ==========
        // Même pattern que MovieDetails/TVDetails : ping via le proxy backend (CORS)
        // avec timeout court, juste pour savoir si l'épisode existe côté Wavewatch —
        // sans ça `applyEmbedConfig('wavewatch')` pourrait auto-sélectionner un lien mort.
        const wavewatchPromise = (async () => {
          try {
            const wwUrl = `https://wwembed.wavewatch.top/api/v1/streaming/ww-tv-${id}-s${seasonNumber}-e${episodeNumber}`;
            const ctrl = new AbortController();
            const timer = setTimeout(() => ctrl.abort(), 8000);
            try {
              const r = await fetch(`${MAIN_API}/proxy/${encodeURIComponent(wwUrl)}`, { signal: ctrl.signal });
              return r.ok ? wwUrl : null;
            } finally {
              clearTimeout(timer);
            }
          } catch (error) {
            console.error('Error checking Wavewatch availability:', error);
            return null;
          }
        })();
        ...
        const applyEmbedConfig = (sourceId: TopLevelSourceId): boolean => {
          switch (sourceId) {
            ...
            case 'wavewatch': {
              if (!wavewatchUrlResult) return false;
              console.log('✅ Selecting WAVEWATCH as source');
              // Convention existante (cf. handleSourceChangeFromMenu) : les lecteurs VO/VOSTFR
              // top-level, Wavewatch inclus, partagent le type générique 'vostfr' côté embed —
              // c'est l'URL qui les distingue, pas selectedSource/embedType.
              setSelectedSource('vostfr');
              setEmbedUrl(wavewatchUrlResult);
              setEmbedType('vostfr');
        ...
        if (!embedseekPromoted) {
          ...
          const availability: SourceAvailability[] = [
            ...
            { id: 'wavewatch', hasData: !!wavewatchUrlResult },
  ...
  if (animeMode && !animeSourceFallback) {
    return (
      <WatchAnime
        id={id}
        initialSeason={String(seasonNumber)}
        initialEpisode={String(episodeNumber)}
        headerTitle={showTitle}
        headerYear={tvMeta?.first_air_date && !isNaN(new Date(tvMeta.first_air_date).getTime()) ? new Date(tvMeta.first_air_date).getFullYear() : undefined}
        headerVoteAverage={tvMeta?.vote_average}
        headerCertification={displayCert}
        headerSeasonsCount={tvMeta?.number_of_seasons}
        recommendations={recommendations}
        onNoSourceFallback={() => setAnimeSourceFallback(true)}
      />
    );
  }
  ...
  return (
    <div className="w-full max-w-4xl mx-auto my-6">
      ...
      <div className="mb-4 flex flex-col items-center text-center gap-3">
        <h1 className="text-2xl md:text-3xl font-bold text-white">{showTitle}</h1>
        <div className="flex flex-wrap items-center justify-center gap-3">
          {tvMeta?.first_air_date && !isNaN(new Date(tvMeta.first_air_date).getTime()) && (
            <span className="flex items-center gap-1.5 text-white/60 text-sm">
              <span>📅</span>
              <span>{new Date(tvMeta.first_air_date).getFullYear()}</span>
            </span>
          )}
          {displayCert && (
            <span className="px-2 py-0.5 rounded text-xs font-bold bg-gradient-to-r from-green-400 to-purple-500 text-white">
              {getClassificationLabel(displayCert, t)}
            </span>
          )}
          {!!tvMeta?.vote_average && tvMeta.vote_average > 0 && (
            <span className="flex items-center gap-1 text-white/60 text-sm">
              <span className="text-yellow-500">⭐</span>
              <span className="font-bold text-white/80">{tvMeta.vote_average.toFixed(1)}</span>
              <span className="text-xs">/10</span>
            </span>
          )}
          {!!tvMeta?.number_of_seasons && tvMeta.number_of_seasons > 0 && (
            <span className="flex items-center gap-1.5 text-white/60 text-sm">
              <span>🎬</span>
              <span>{tvMeta.number_of_seasons} saison{tvMeta.number_of_seasons > 1 ? 's' : ''}</span>
            </span>
          )}
          {tvMeta?.first_air_date && !isNaN(new Date(tvMeta.first_air_date).getTime()) ? (
            new Date(tvMeta.first_air_date) > new Date() ? (
              <span
                className="inline-flex items-center px-4 py-1 rounded-full text-xs font-semibold tracking-wider uppercase"
                style={{ background: 'rgba(113,63,18,0.3)', color: '#fbbf24', border: '1px solid rgba(251,191,36,0.35)', boxShadow: '0 0 10px rgba(251,191,36,0.15)' }}
              >
                {t('details.upcomingBadge')}
              </span>
            ) : (
              <span
                className="inline-flex items-center px-4 py-1 rounded-full text-xs font-semibold tracking-wider uppercase"
                style={{ background: 'rgba(22,101,52,0.25)', color: '#4ade80', border: '1px solid rgba(74,222,128,0.4)', boxShadow: '0 0 12px rgba(74,222,128,0.25), inset 0 0 8px rgba(74,222,128,0.05)' }}
              >
                {t('details.releasedBadge')}
              </span>
            )
          ) : (
            <span
      ...
      {isLoading ? (
        ...
        <div className="h-full bg-black text-white flex flex-col relative">
          ...
          {/* Sources dispo affichees directement, sans ecran d'avertissement intermediaire */}
          <div className="w-full h-full overflow-y-auto pt-16 px-4 pb-4">
            <h3 className="text-white text-lg font-bold mb-4">{t('watch.changeSource')}</h3>
            <HLSPlayer
        ...
        <div className="w-full h-full flex flex-col">
          {/* Barre au-dessus du player : Retour / Précédent / Épisodes / Suivant */}
          <div className="relative z-50 flex items-center justify-between gap-2 px-4 py-3 flex-shrink-0">
          <button
            onClick={() => navigate(`/tv/${encodeId(id!)}`)}
            className="flex items-center gap-2 px-3 py-2 rounded-xl text-white transition-all duration-200"
          ...
          {nextEpisodeData && (
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="flex items-center justify-center px-3 py-2 rounded-xl text-white transition-all duration-200"
                style={GLASS_BTN_STYLE}
          ...
          <AnimatePresence>
            {showEmbedQuality && (
              <motion.div
                ...
                className="absolute inset-0 z-[10001] bg-black/60 flex justify-end pointer-events-auto"
              >
      ...
      {/* Encadré infos de la série sélectionnée — sous le player, au-dessus des recommandations */}
      <div className="mt-6 rounded-2xl border border-white/10 bg-white/[0.03] p-6 flex flex-col sm:flex-row sm:items-stretch gap-6">
        {showPosterPath && (
          <div className="relative w-32 sm:w-40 md:w-48 flex-shrink-0 mx-auto sm:mx-0">
            <img
              src={`https://image.tmdb.org/t/p/w342${showPosterPath}`}
              alt={showTitle}
        ...
        <div className="flex-1 min-w-0">
          {tvMeta?.overview && (
            <p className="text-white/60 text-sm md:text-base leading-relaxed">{tvMeta.overview}</p>
          )}
        </div>
      </div>
      ...
      {recommendations.length > 0 && (
        <div className="mt-8">
          <div className="mb-4 flex items-center justify-center gap-2">
            ...
            <h3 className="text-xl font-bold text-white">Séries similaires</h3>
          </div>
          <div className="flex gap-3 justify-center">
            {recommendations.slice(0, 4).map((rec: any) => (
              <Link
                key={rec.id}
                to={`/tv/${encodeId(rec.id)}`}
