...
import { sortHostersByPriority, type SourceAvailability } from '../../utils/sourceAutoSelect';
...
const WatchTv: React.FC = () => {
  ...
  const [similarContent, setSimilarContent] = useState<{ id: number; title?: string; name?: string; poster_path: string; vote_average: number; media_type?: string }[]>([]);
  ...
  const [vfNotificationVisible, setVfNotificationVisible] = useState(false);
  const [vfReadySources, setVfReadySources] = useState<TopLevelSourceId[]>([]);
  const applyVfConfigRef = useRef<(() => void) | null>(null);
    ...
    try {
      ...
      if (nextEpisodeInCurrentSeason) {
        ...
        if (nextSeason) {
          ...
          setNextEpisodeData(null);
          // Série terminée → charger du contenu similaire
          if (id) {
            try {
              ...
              const results = (similarRes.data?.results || [])
                .filter((r: any) => r.poster_path)
                .slice(0, 6)
                .map((r: any) => ({ ...r, media_type: 'tv' }));
  ...
  useEffect(() => {
    ...
    const initialFetch = async () => {
      ...
      try {
        ...
        // Immediately launch VOSTFR player while VF sources load in background
        setEmbedUrl(`https://vidlink.pro/tv/${id}/${seasonNumber}/${episodeNumber}`);
        setEmbedType('vostfr');
        ...
        const wiflixPromise = (async () => {
          try {
            const wiflixResponse = await axios.get(`${MAIN_API}/api/wiflix/tv/${id}/${seasonNumber}`, { timeout: 30000 });
        ...
        const viperPromise = (async () => {
          try {
            const viperResponse = await axios.get(`${MAIN_API}/api/cpasmal/tv/${id}/${seasonNumber}/${episodeNumber}`, { timeout: 30000 });
        ...
        console.log('🇫🇷 [VF] Darkino disponible:', darkinoResult && typeof darkinoResult === 'object' ? darkinoResult.available : false, '| Sources:', darkinoResult && typeof darkinoResult === 'object' ? darkinoResult.sources?.length : 0);
        console.log('🇫🇷 [VF] FStream sources:', fstreamProcessedSources.length, fstreamProcessedSources.map((s: any) => s.label || s.url));
        console.log('🇫🇷 [VF] Wiflix/Lynx sources:', wiflixProcessedSources.length, wiflixProcessedSources.map((s: any) => s.label || s.url));
        console.log('🇫🇷 [VF] Viper/CPasMal sources:', viperProcessedSources.length, viperProcessedSources.map((s: any) => s.label || s.url));
        console.log('🇫🇷 [VF] Coflix disponible:', !!(coflixResult && getMultiFromCoflix(coflixResult)));
        console.log('📦 [HLS] Nexus HLS sources:', finalHlsSources.length, finalHlsSources.map((s: any) => s.label || s.url));
        console.log('📦 [FILE] Nexus File sources:', finalFileSources.length, finalFileSources.map((s: any) => s.label || s.url));
        console.log('📦 [MP4] MP4 sources:', fetchedMp4Sources.length);
        console.log('📦 [OMEGA] Omega disponible:', !!(omegaResult));
        console.log('📦 [ADFREEK] AdFree M3U8:', adFreeM3u8Url);
        console.log('📦 [CUSTOM] Custom links:', customLinksResult.customLinks?.length || 0);
        ...
        if (!embedseekPromoted) {
          ...
          // === PRIORITÉ VF : notifier l'utilisateur si des sources VF sont disponibles ===
          // VOSTFR est déjà en lecture — on ne switche pas automatiquement, on notifie.
          const vfCandidates: TopLevelSourceId[] = ['darkino', 'fstream', 'wiflix', 'coflix', 'viper'];
          const vfAvailable = vfCandidates.filter(srcId => availability.find(a => a.id === srcId && a.hasData));
          console.log('🇫🇷 [VF Priority] Sources VF disponibles:', vfAvailable.length > 0 ? vfAvailable.join(', ') : 'aucune');

          if (vfAvailable.length > 0) {
            // Stocker le callback de switch VF dans un ref pour usage depuis la notification
            applyVfConfigRef.current = () => {
              for (const vfId of vfAvailable) {
                if (applyEmbedConfig(vfId)) break;
              }
              setVfNotificationVisible(false);
            };
            setVfReadySources(vfAvailable);
            setVfNotificationVisible(true);
            console.log('✅ [VF Priority] Notification affichée pour:', vfAvailable.join(', '));
          } else {
            console.log('❌ Aucune source VF disponible, VOSTFR reste actif');
          }
        }
  ...
  // Auto-redirect vers le lecteur VOSTFR (Vidlink) quand aucune source VF n'est trouvée
  useEffect(() => {
    if (onlyVostfrAvailable && id && seasonNumber && episodeNumber) {
      setEmbedUrl(`https://vidlink.pro/tv/${id}/${seasonNumber}/${episodeNumber}`);
      setEmbedType('vostfr');
      setOnlyVostfrAvailable(false);
    }
  }, [onlyVostfrAvailable, id, seasonNumber, episodeNumber]);
    ...
    <div style={{ minHeight: 'calc(var(--vh, 1vh) * 100)' }} className="w-full bg-black text-white overflow-x-hidden lg:fixed lg:inset-0 lg:overflow-hidden">
      ...
      {isLoading ? (
                ...
                <div className="bg-black/95 border-l border-gray-800 shadow-2xl w-full max-w-md h-full overflow-y-auto z-[10002]">
                  <div className="flex justify-between items-center p-4 border-b border-gray-700/60 sticky top-0 bg-black/95 z-10">
                    ...
                    <button
                      ...
                      className="text-gray-400 hover:text-green-400 transition-colors text-2xl font-bold focus:outline-none"
                    >
                      ×
                    </button>
                  </div>
        ...
        <div className="w-full h-full flex flex-col items-center justify-center relative">
          ...
          {/* Notification Sources VF disponibles */}
          <AnimatePresence>
            {vfNotificationVisible && (
              <motion.div
                key="vf-notification"
                initial={{ opacity: 0, y: -20 }}
                animate={{ opacity: 1, y: 0 }}
                ...
                className="fixed top-20 left-1/2 -translate-x-1/2 z-[9999] flex items-center gap-3 px-4 py-3 rounded-xl bg-black/90 border border-green-500/60 shadow-2xl shadow-green-500/20"
                style={{ minWidth: '280px', maxWidth: '90vw' }}
              >
                <span className="text-green-400 text-xl">🇫🇷</span>
                <div className="flex-1">
                  <p className="text-white font-semibold text-sm">Sources VF disponibles</p>
                  <p className="text-gray-400 text-xs">{vfReadySources.join(', ')}</p>
                </div>
                <button
                  onClick={() => applyVfConfigRef.current?.()}
                  className="px-3 py-1.5 rounded-lg bg-green-500 hover:bg-green-400 text-black font-bold text-xs transition-colors"
                >
                  Regarder en VF
                </button>
                <button
                  onClick={() => setVfNotificationVisible(false)}
        ...
        <div className="w-full h-full flex items-center justify-center relative">
          <HLSPlayer
            ...
            nextContentThreshold={{ mode: 'timeBeforeEnd', value: 30 }}
            similarContent={similarContent}
            onSimilarContentSelect={(contentId, mediaType) => {
              const encoded = encodeId(String(contentId));
              if (mediaType === 'tv') {
                window.location.href = `/tv/${encoded}`;
              } else {
                window.location.href = `/movie/${encoded}`;
              }
            }}
          />
