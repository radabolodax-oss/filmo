  ...
  return (
    <div style={{ minHeight: 'calc(var(--vh, 1vh) * 100)', overflow: 'hidden' }} className="w-full bg-gray-900 text-white overflow-hidden fixed inset-0">
      ...
      {!id ? (
            ...
            <div className="mt-4 space-y-2">
              ...
              {loadingOneUploadExtraction && (
                <div className="flex items-center gap-2 text-white text-sm">
                  <div className="w-3 h-3 border-2 border-white/20 border-t-transparent rounded-full animate-spin"></div>
        ...
        <div className="flex items-center justify-center h-full">
          <div className="max-w-2xl mx-auto bg-gray-800 p-8 rounded-xl shadow-2xl">
            <h2 className="text-2xl font-bold text-green-400 mb-4">{t('watch.errorTitle')}</h2>
                ...
                >
                  ...
                  <div className="p-4">
                          ...
                          {availableLanguages.map(lang => (
                            <div
                              ...
                              className={`relative flex items-center rounded-lg transition-all duration-200 ${selectedLanguage === lang
                                ? 'bg-gray-800 border-l-4 border-white/20 pl-3 font-medium'
                    ...
                    <div className="mb-6">
                      ...
                      {selectedSource && (
                        <div className="bg-gray-800/60 rounded-lg p-3 mb-4 border-l-4 border-white/20">
                          <div className="flex items-center justify-between">
                            <div>
                              <p className="text-green-400 text-sm font-medium">{t('watch.currentSource')}</p>
                          ...
                          .map((source, index) => (
                            <motion.button
                              ...
                              className={`w-full px-4 py-3 text-sm text-left hover:bg-gray-800/80 rounded-lg mb-2 flex justify-between items-center ${selectedSource?.id === source.id
                                ? 'bg-gray-800 border-l-4 border-white/20 pl-3'
                                : 'bg-gray-900/60 text-white'
                                }`}
                            >
                              <div className="flex flex-col">
                                <span className={selectedSource?.id === source.id ? 'text-green-400 font-medium' : 'text-white'}>
                                  {source.player}
                                </span>
                              ...
                              {selectedSource?.id === source.id && (
                                <span className="text-xs px-2 py-1 bg-gradient-to-r from-green-400 to-purple-500 text-white rounded-full">{t('watch.active')}</span>
                              )}
                            </motion.button>
                          ))}
                      </div>
