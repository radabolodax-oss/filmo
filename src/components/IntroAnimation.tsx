...
const IntroAnimation: React.FC<IntroAnimationProps> = ({ onAnimationComplete }) => {
  ...
  useEffect(() => {
    ...
    t(() => { setStep(3); triggerFlash('#ffffff', 300); vibe(VIBE.soft, 'soft'); }, 3200);
                    ...
                    {(allParticles[i as keyof typeof allParticles] || []).map((p, pi) => (
                      <div
                        ...
                        style={{
                          width: p.size,
                          height: p.size,
                          backgroundColor: i === 3 ? '#ffffff' : '#22c55e',
                          boxShadow: `0 0 ${p.size * 3}px ${i === 3 ? 'transparent' : 'rgba(34,197,94,0.8)'}`,
                    ...
                    {(isMobile ? [0, 1] : [0, 1, 2, 3]).map(si => (
                      <div
                        ...
                        style={{
                          ...
                          backgroundColor: i === 3 ? 'transparent' : 'rgba(34,197,94,0.12)',
                        }}
                      />
                    ))}
                  </div>
                ...
                {step >= i && (
                  <div
                    ...
                    style={{
                      ...
                      border: `1.5px solid ${i === 3 ? 'transparent' : 'rgba(34,197,94,0.45)'}`,
                  ...
                  >
                    ...
                    <div className="absolute inset-0 rounded-sm pointer-events-none"
                      style={{
                        background: `radial-gradient(ellipse at 50% 30%, ${i === 3 ? 'transparent' : 'rgba(34,197,94,0.08)'} 0%, transparent 70%)`,
                      }}
                    />

                    {/* Atomic number */}
                    <span className={`absolute top-1 left-1.5 sm:top-1.5 sm:left-2 text-[7px] sm:text-[10px] md:text-xs font-mono font-bold
                      ${i === 3 ? 'text-white' : 'text-green-400/80'}`}>
                      {el.number}
                    </span>
                    ...
                    <span
                      className={`text-lg sm:text-3xl md:text-5xl font-bold leading-none mt-1 sm:mt-2
                        ${i === 3 ? 'text-white' : 'text-green-300'}`}
                      style={{
                        textShadow: `0 0 20px ${i === 3 ? 'transparent' : 'rgba(34,197,94,0.6)'},
                                     0 0 40px ${i === 3 ? 'transparent' : 'rgba(34,197,94,0.2)'}`,
                    ...
                    <span className={`text-[5px] sm:text-[8px] md:text-[9px] mt-0.5 sm:mt-1.5 tracking-[0.1em] sm:tracking-[0.15em] uppercase font-mono
                      ${i === 3 ? 'text-white' : 'text-green-400/50'}`}>
                      {el.name}
                    </span>

                    {/* Atomic mass */}
                    <span className={`hidden sm:inline text-[7px] font-mono mt-0.5
                      ${i === 3 ? 'text-white' : 'text-green-400/30'}`}>
                      {el.mass}
                    </span>
                    ...
                    {[
                      ...
                      <div key={pos}
                        className={`absolute w-1.5 h-1.5 sm:w-3 sm:h-3 ${pos} ${i === 3 ? 'border-white/20' : 'border-green-400/25'}`}
                      />
                    ))}
                  </div>
                )}
              ...
              <div className="relative w-[50px] h-[66px] sm:w-[88px] sm:h-[112px] md:w-[105px] md:h-[132px]
                ...
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-20">
                  {SPARKLE_POSITIONS.map((sp, i) => (
                    <div key={i}
                      className="absolute w-1.5 h-1.5 bg-gradient-to-r from-green-400 to-purple-500 rounded-full intro-sparkle"
                      style={{
                        left: `${sp.x}%`,
                        top: `${sp.y}%`,
