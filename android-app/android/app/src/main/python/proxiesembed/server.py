...
# MOBILE PORT: aiomysql dropped from requirements-mobile.txt -- the VIP/
# account/payment system this powered (_init_mysql(), see near start_server())
# was already removed from Mainapi in an earlier cleanup, so this DB is dead
# weight tied to a feature that no longer exists. _init_mysql() is not called
# below; self.mysql_pool stays None, which every call site already handles.
# import aiomysql
import os
import builtins
import traceback
...
# MOBILE PORT: renamed from '.env' to 'env.production' -- AAPT/Chaquopy asset
# packaging can silently drop dotfiles from packaged source (same issue hit
# porting API/Mainapi's .env, see android-app/scripts/zip_node_backend.py and
# the nodejs-project main.js header comment for the established fix pattern).
...
# Domains allowed through /embed-proxy (security whitelist)
EMBED_PROXY_ALLOWED = frozenset([
    'vidzy.org', 'v1.vidzy.org', 'v2.vidzy.org', 'v3.vidzy.org', 'v4.vidzy.org',
    'player.videasy.net', 'wwembed.wavewatch.top',
    'vidmoly.to', 'getromes.space', 'vmwesa.online',
    'voe.sx', 'uqload.co', 'uqload.cx', 'uqload.com',
    'doodstream.com', 'd0000d.com',
    'sibnet.ru', 'video.sibnet.ru',
    'premiumembeds.xyz', 'seekstreaming.net', 'embedseek.xyz',
    'fsvid.lol', 'fsvid.net',
    'cineplex.to', 'cinep.lol',
    'franime.fr',
])

# Anti-ad + anti-popup script injected into proxied embed HTML pages
AD_BLOCK_SCRIPT = b'''<script data-movix="anti-ad">
(function(){
  // --- Popup blocker ---
  var noop = function(){ return {focus:function(){},close:function(){},blur:function(){},document:{write:function(){},writeln:function(){}},location:{}}; };
  window.open = noop;
  ...
  // --- Ad domain blocklist ---
  var AD=['doubleclick.net','googlesyndication.com','googletagmanager.com','adnxs.com',
          'outbrain.com','taboola.com','pubmatic.com','openx.net','rubiconproject.com',
          'criteo.com','2mdn.net','moatads.com','advertising.com','adform.net',
          'smartadserver.com','tradedoubler.com','adtech.de','yieldmo.com',
          'sharethrough.com','triplelift.com','33across.com','sonobi.com','adsystem'];
  function isAd(u){try{var s=String(typeof u==='string'?u:(u&&u.url)||'');return AD.some(function(d){return s.indexOf(d)!==-1;});}catch(e){return false;}}
  // Block fetch
  if(window.fetch){var f0=window.fetch;window.fetch=function(u,o){return isAd(u)?Promise.resolve(new Response('',{status:204})):f0.apply(this,arguments);};}
  // Block XHR
  var XP=window.XMLHttpRequest&&window.XMLHttpRequest.prototype,XO=XP&&XP.open;
  if(XO){XP.open=function(m,u){if(!isAd(u))XO.apply(this,arguments);};}
  // --- CSS overlay hider ---
  var CSS=[
    '[id*="pop"],[class*="pop"]:not(video):not(audio)',
    '[id*="over"],[class*="over"]:not(video):not(.overlay-controls)',
    '[id*="banner"],[class*="banner"]:not([class*="player"])',
    '[id*="modal"]:not([class*="player"])',
    '[data-format="inread"],[data-ad],[data-adunit]',
    '[id*="sponsor"],[class*="sponsor"]',
    'div[style*="z-index: 9999"]:not(video):empty',
    '.ad,.ads,.adsbygoogle,[class^="ad_"],[id^="ad_"]'
  ].join(',');
  function injectCss(){
    var s=document.createElement('style');
    s.textContent=CSS+'{display:none!important;visibility:hidden!important;opacity:0!important;pointer-events:none!important;max-height:0!important;overflow:hidden!important;}';
...
class ProxyServer:
    ...
    def setup_routes(self):
        ...
        self.app.router.add_get('/embed-proxy', self.embed_proxy_handler)
    ...
    async def _check_vip(self, request: Request) -> bool:
        ...
        MOBILE PORT: short-circuited to always return True. Discovered during
        on-device testing: src/utils/vipUtils.ts's getVipHeaders()/isUserVip()
        are permanent no-op stubs ("VIP/access-key system removed (account +
        payment + access-key VIP all gone)") -- the frontend never sends an
        x-access-key header anywhere, anymore, on desktop OR mobile. Since
        this method returns False whenever the header is missing (see below,
        unreached now), every /api/extract-*, /api/voe/m3u8 etc. call was
        unconditionally getting a 403 VIP_REQUIRED -- i.e. this orphaned gate
        (left over from the same VIP/account/payment cleanup that removed
        _init_mysql()'s reason to exist, see the aiomysql import comment
        above) was blocking 100% of real extraction requests, on the
        unmodified desktop server.py too, not just here. Confirmed via a
        direct on-device curl to /api/extract-uqload and /api/voe/m3u8
        returning {"error": "VIP access required"} with zero headers, exactly
        matching what the real frontend sends. Not fixing this would leave
        the whole point of this Chaquopy port (real extraction) permanently
        inert, so this is patched here -- matching the already-established
        "copy + adapt, don't mutate the original" latitude for this port.
        """
        return True
    ...
    async def embed_proxy_handler(self, request: Request) -> Response:
        """Fetch an embed HTML page server-side, inject anti-ad script, return modified HTML."""
        ...
        try:
            ...
            if not any(domain == d or domain.endswith('.' + d) for d in EMBED_PROXY_ALLOWED):
                return Response(status=403, text='Domain not allowed')
        except Exception:
            return Response(status=400, text='Invalid URL')
        try:
            headers = {
            ...
            async with session.get(url, headers=headers, timeout=ClientTimeout(total=12), allow_redirects=True) as resp:
                ...
                if 'html' in ct:
                    if b'</head>' in content:
                        content = content.replace(b'</head>', AD_BLOCK_SCRIPT + b'</head>', 1)
                    elif b'<body' in content:
                        idx = content.find(b'<body')
                        end = content.find(b'>', idx)
                        if end != -1:
                            content = content[:end + 1] + AD_BLOCK_SCRIPT + content[end + 1:]
                    else:
                        content = AD_BLOCK_SCRIPT + content
                out_headers = dict(CORS_HEADERS)
                out_headers['Content-Type'] = ct
                out_headers['X-Frame-Options'] = 'ALLOWALL'
                return Response(body=content, headers=out_headers)
        except Exception as e:
            logger.warning(f'[embed-proxy] error fetching {url}: {e}')
            return Response(status=502, text=f'Proxy error: {e}')
    ...
    async def start_server(self):
        ...
        # MOBILE PORT: handle_signals=False -- this runs on a Chaquopy
        # background thread, not the interpreter main thread, and
        # loop.add_signal_handler() raises ValueError off-main-thread
        # (aiohttp's own internal handling only swallows NotImplementedError,
        # the Windows case, not this). Android kills the process on app
        # exit, so there is no Ctrl+C-style shutdown to handle anyway.
        ...
        await runner.setup()
        # MOBILE PORT: MySQL VIP verification disabled -- see aiomysql import
        # comment above. self.mysql_pool stays None (set in __init__).
        ...
        # MOBILE PORT: bind to loopback only, matching the embedded Mainapi's
        # security posture (server-mobile.js also binds 127.0.0.1) -- this
        # server is only ever reached from the on-device WebView, never from
        # the network.
        site = web.TCPSite(
            runner,
            '127.0.0.1',
...
async def main():
    ...
    # MOBILE PORT: this runs on a Chaquopy-invoked background Java thread,
    # not the interpreter's main thread -- signal.add_signal_handler() only
    # works on the main thread of the main interpreter, so guard it instead
    # of letting an unhandled ValueError kill server startup. Desktop
    # (always launched as `python server.py` on the main thread) is
    # unaffected -- this is the same "graceful degrade" pattern already
    # used for optional widefrog/service init elsewhere in this file.
    if sys.platform != 'win32':
        try:
            ...
            logger.warning(f'Signal handlers not installed (non-main thread?): {e}')
